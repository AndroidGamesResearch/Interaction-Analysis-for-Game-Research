# =====================================================================
# IAP × SPORTS — WORKED INTERACTION ANALYSIS
# Supplementary reproducibility material
#
# This script generates the complete IAP × Sports worked example used to
...
OUTPUT_FILE = "iap_sports_worked_example.pdf"
...
def calculate_interaction(data, outcome):
    ...
    if len(focal) == 0:
        raise ValueError("No Sports observations in this context.")

    if len(other) == 0:
        raise ValueError("No non-Sports observations in this context.")
...
def page_footer(
    ...
    fig.text(
        LEFT,
        0.026,
        "Supplementary worked example  |  IAP × Sports interaction analysis",
        fontsize=5.9,
        color=MUTED,
        ha="left"
    )
...
def render_page_1(pdf):
    ...
    page_header(
        fig,
        "Worked example: IAP × Sports interaction",
    ...
    contingency_booktabs(
        ...
        f"Sports stratum  (n = {R['a1']+R['b1']+R['c1']+R['d1']:,})",
        R["a1"], R["b1"], R["c1"], R["d1"]
    )
    ...
    contingency_booktabs(
        ...
        f"Non-Sports stratum  (n = {R['a0']+R['b0']+R['c0']+R['d0']:,})",
        R["a0"], R["b0"], R["c0"], R["d0"]
    )
...
def render_page_8(pdf):
    ...
    page_header(
        fig,
        "Complete IAP × Sports interaction summary",
...
def render_page_9(pdf):
    ...
    page_header(
        ...
        "IAP × Sports interaction  |  All national markets"
    )
    ...
    fig.text(
        ...
        0.070,
        (
            "Each row reports the country-specific IAP × Sports interaction estimate "
...
def render_page_10(pdf):
    ...
    page_header(
        ...
        "IAP × Sports interaction  |  Install success and rating success"
    )
    ...
    note(
        ...
        (
            ...
            "zero indicate that the IAP-associated probability difference is "
            "more negative for Sports than for non-Sports games; values above zero "
